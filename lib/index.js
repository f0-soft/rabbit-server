...
function startFind (query){
    ...
    if(options.sortData && options.sortData.add &&!options.sortExt){
        if(!selector.$and)  selector.$and = [];

        var fieldSort = _.keys(query.options.sort);

        var resObj = {};
        resObj[fieldSort[0]] = options.sortData.add;
        selector.$and.push(resObj);
    }
    ...
    if(query.fields){
        for(var i = 0; i < query.fields.length ; i++){
            if(query.fields[i].indexOf('.') !== -1){
                fields['_a.'+query.fields[i].replace('.','__')] = 1;
            } else if(gArrayFields[collname] && gArrayFields[collname][query.fields[i]] && query.fields[i] != '_a'){
                if(gArrayFields[collname][query.fields[i]] == 1) {
                    fields['_a.'+query.fields[i]] = 1;
                } else {
                    var subkeys = Object.keys(gArrayFields[collname][query.fields[i]]);
                    for(var n = 0; n < subkeys.length; n++){
                       fields['_a.'+query.fields[i]+'__'+subkeys[n]] = 1;
                    }
                }
    ...
    if(options.sortExt){
        ...
        options.sepcount = JSON.stringify(selector);
    ...
    if(options.sortExt && gBuffScore[options.sortExt.collnameExt]){
        ...
        arrayParAsync.push(function (cb){
            mongoFind.call(self, queryFind, cb);
        });

        if(queryFind.options.sortData && queryFind.options.sortData.count){
            var queryFindSort = {
                selector: queryFind.selector,
                fields: fields,
                options:{
                    limit: 1,
                    skip: queryFind.options.sortData.count+90,
                    sort: queryFind.options.sort
                }
            }

            arrayParAsync.push(function (cb){
                mongoFind.call(self, queryFindSort, cb);
            });
        }

        arrayRes.push(function (cb){
            async.parallel(arrayParAsync, function (err, res){
                if(err) cb(err);
                else {
                    if(res[1] && res[1][0].length > 0){
                        var sortField = _.keys(queryFind.options.sort);
                        var result = null;
                        if(res[1][0][0][sortField[0]]) result = res[1][0][0][sortField[0]];
                        res[0].push(result);
                        cb(null, res[0]);
                    } else cb(null, res[0]);
                }
...
function updateSelector (collname, selector){
    ...
    if(gAllFieldDepPath[collname]){
        pathFields =  Object.keys(gAllFieldDepPath[collname]);
    }
...
function findSortExt (query, callback){
    ...
    mongoFind.call(self, queryFind, function(err, res){
        rCount = res[1];
        var alpath;
        if(options.sort[fesc] === 1) alpath = maxScroe({collname: optionsExt.collnameExt, field: fescL, _a: res[0][res[0].length-1]['_a']});
        else alpath = minScroe({collname: optionsExt.collnameExt, field: fescL, _a: res[0][res[0].length-1]['_a']});

        if (!alpath) mongoFind.call(self, query, callback);
        else {
            findSortExtUpdate.call(self, query, function (error, res2){
                if(error) callback(error);
                else {
                    query.selector._id = {$in: res2[0]};
                        ...
                        if(error) callback (error);
                            ...
                            for(var i = 0; i < res2[0].length; i++){
                                for(var z = 0; z < result[0].length; z++){
                                    if(res2[0][i] === result[0][z]._id){
                                        sortRes.push(result[0][z]);
                                        break;
                                    }
                                }
                            }
                            var countRes = null;
                            if(options.count) countRes = res2[1];
...
function findSortJoin (query, callback){
    ...
    mongoFind.call(self, query, function(err, res){
        if(err) callback (err);
        else {
            if(!_.isNull(res[1])) {
                delete  query.options.count;
                query.countRes = res[1];
            }
            parseScore.call(self, query, res[0], callback);
        }
    });
}
...
//    var fscore = query.fesc;
    ...
    if(newScore.length > 0) {
        ...
        mongoFind.call(self, findQuery, function(err, result){
            if(err) callabck (err);
            else {
                result = result[0];
    ...
    if(query.options && query.options.count ==  1){
        ...
        async.parallel({
                ...
                count: function(callback){
                    if(query.options.sepcount) {
                        var sel = JSON.parse(query.options.sepcount);
                        var cusrorCount = self.coll.find(sel, query.fields, {limit:1});
                        cusrorCount.count(callback);
                    }
                    else{
...
//    console.log('gModifyNewPath', gModifyNewPath);
    if(gFieldDepPath[collname]){
        var fields = Object.keys(gFieldDepPath[collname]);
...
function endFind (err, results){
    if(err) this.callback(err);
    else {
        var res = {};
        res.result = updateResult(this.collname, results[0], this.flag_a);
        ...
        if(!_.isNull(results[1])) res.count = results[1];
        if(results[2]) res.sortData = results[2];
...
function add_a (collname, obj){
    if(gArrayFields[collname]){
        ...
        var field = Object.keys(obj);
        for(var q = 0; q < field.length; q++){
            if(field[q].indexOf('.') !== -1){
                console.log(field[q]);
                obj['_a.'+field[q].replace('.','__')] = obj[field[q]];
                delete obj[field[q]];
            }
        }

        var keys = Object.keys(gArrayFields[collname]);

        for(var i = 0; i < keys.length; i++){
            if(obj[keys[i]]){
                if(gArrayFields[collname][keys[i]] == 1){
                    obj['_a.'+keys[i]] = obj[keys[i]];
                } else {
                    var subkeys = Object.keys(gArrayFields[collname][keys[i]]);
                    if(obj[keys[i]].$elemMatch){
                        var resObj = {_a:{}};
                        for(var n = 0; n < subkeys.length; n++){
                            if(obj[keys[i]].$elemMatch[subkeys[n]]){
                                resObj._a[keys[i]+'__'+subkeys[n]] = obj[keys[i]].$elemMatch[subkeys[n]];
                                delete obj[keys[i]].$elemMatch[subkeys[n]];
                            }
                        }
                        if(!obj.$and) obj.$and = [];
                        obj.$and.push(resObj);
    ...
    if(gWordFields[collname]){
        var keys = Object.keys(gWordFields[collname]);
    ...
    if(data[0] && data[0]['_a']){
        for(var z = 0; z < data.length; z++){
            for(var i = 0; i < data[z]['_a'].length; i++){
                var key = Object.keys(data[z]['_a'][i]);
                if(key.length > 0 && key[0].indexOf('__') > 0){
                    var fields = key[0].split('__');
                    if(!data[z][fields[0]]) data[z][fields[0]] = [];
                    var objRes = {};
                    for(var v = 0; v < key.length; v++){
                        fields = key[v].split('__');
                        objRes[fields[1]] = data[z]['_a'][i][key[v]];
                    }
                    data[z][fields[0]].push(objRes);
                    delElem.push(i);
                } else if(key.length > 0 && aField && aField[key[0]]){
...
function resUnderscore (collname, data){
    var keys = []
    if(gFieldCollStart[collname]) keys = keys.concat(Object.keys(gFieldCollStart[collname]));
    if(gFieldDepPath[collname]) keys = keys.concat(Object.keys(gFieldDepPath[collname]));
        ...
        if(doc[keys[i]]){
            ...
            if(_.isArray(doc[keys[i]])){
                if(gArrayFields[collname][keys[i]] == 1){
                    ...
                    for(var n = 0 ; n < doc[keys[i]].length ; n++){
                        var subkeys = Object.keys(doc[keys[i]][n]);
                        var obj1 = {};
                        for(var m = 0; m < subkeys.length; m++){
                           obj1[keys[i]+'__'+subkeys[m]] = doc[keys[i]][n][subkeys[m]];
                        }
                        doc['_a'].push(obj1);
                    }
                }
...
function createPath (doc, callback){
    var key = Object.keys(gFieldDepPath[this.collname]);
    var self = this;
    var aQuery = [];
...
//        aWaterfall.push(function (res, cb){
//            var keys = Object.keys(gScoreJoin[collname]);
//            var fields = [];
//            for(var i = 0; i < keys.length; i++){
//                var subkeys = Object.keys(gScoreJoin[collname][keys[i]]);
//                for(var z = 0; z < subkeys.length; z++){
//                    var sfiled = Object.keys(gScore[subkeys[z]]);
...
function checkUpdatePath (query, callback){
    ...
    var keys = Object.keys(gAllFieldDepPath[collname]);
    var fields = [];
    var collnamePath = [];
    var namePath = [];
    var indexCollPath = [];
...
function findDepDocModify (path, id){
    ...
    if(gModifyNewPath[path]){
        var aId = Object.keys(gModifyNewPath[path]);
        for(var i = 0; i < aId.length; i++){
...
function deleteDataPath (namepath, id){
    var keys = Object.keys(gModifyPath[namepath]);
    for(var i = 0; i < keys.length; i++){
        var newArrayAdd = [];
...
function extend_a(resFind, doc){
    ...
    if(resFind.sub_a){
        ...
        for(var i = 0; i < resFind.sub_a.length; i++){
            var keys = Object.keys(resFind.sub_a[i]);
...
function startCreateSortIndex (query){
    ...
    aWatrfall.push(function (res, cb){
        ...
        var count = 0;
        var stepLimit = 1000;
        var step = parseInt(maxInt/(res.count+1));

        async.whilst(
            function () { return count < res.count; },
            function (callback) {
                ...
                var queryFind = {
                    ...
                    selector: {},
                    fields: [field],
                    options: {sort:{}, skip: count}
                };
                count = count + stepLimit;

                queryFind.options.sort[query.field] = 1;
                queryFind.options.limit = count;

                find(queryFind, function (error, data){
                    if(error) callback (error);
                    else {
                        var aVal = [];
                        for(var i = 0 ; i < data.result.length; i++){
                            aVal.push(data.result[i][field]);
                        }

                        var unVal = _.union(aVal);

                        for (var i = 0; i < data.result.length; i++){
                            var obj = {selector:{_id: data.result[i]._id}, properties:{}};
                            var index = _.indexOf(unVal,  data.result[i][field]);
                            obj.properties['_s_'+field] = step*(count-stepLimit+index+1);
                            aScore.push(obj);
                        }
                        callback(null, null);
                    }
...
function addScore (doc, callback){
    var keys = Object.keys(gScore[this.collname]);
    var aQuery = [];
    for(var i = 0; i < keys.length; i++){
        if(doc[keys[i]]){
            var obj = {
...
function addJoinScore (doc, callback){
    ...
    var keys = Object.keys(gScoreJoin[this.collname]);
    var aQuery = [];
    for(var i = 0; i < keys.length; i++){
        if(doc[keys[i]]) {
            var subkeys = Object.keys(gScoreJoin[this.collname][keys[i]]);
            for(var z = 0; z < subkeys.length; z++){
                aQuery.push({
                    collname: subkeys[z],
                    field: keys[i],
...
function findJoinScore (query, callback){
    ...
    var fields = Object.keys(gScore[collname]);
...
var  aggregate = function (query, callback){
    ...
    query.pipeline[0].$match = updateSelector(collectionName, query.pipeline[0].$match);
...
//db.bills.aggregate({ $match : {$and: [{_a:{$elemMatch: {p_id:{'$in':[/^prt3aq_/]}}}}]}}, {$project:{sum:1, '_a.p_id':1}}, {$unwind: '$_a'});
