...
var server = net.createServer(function (socket) {
    ...
    socket.on('data', function (data) {
        JSON.parse(data);
        routeMessages(JSON.parse(data), function(err, res){
            var result = {err: null, res: null};
...
function routeMessages (data, callback){
    if(!data.f) callback(myErr('в сообщение от клиента data.f - отсутсвует.'));
    if(!data.й) callback(myErr('в сообщение от клиента data.й - отсутсвует.'));
    switch (data.f) {
        case 'find': {find(data.q, callback); break;}
        case 'insert': {insert(data.q, callback); break;}
        case 'modify': { modify(data.q, callback); break;}
        case 'remove': {remove(data.q, callback); break;}
        default: {callback(myErr('такой функции не существует'));}
    }
...
//    if(options.sortExt ){
//        var keys = _.keys(gModifyPath['manPath']);
//        for(var i = 0; i < keys.length; i++){
//            console.log('keys', keys[i]);
//            console.log(gModifyPath['manPath'][keys[i]]);
//        }
//    }
